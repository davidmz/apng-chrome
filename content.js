(function () {
    ...
    var onImageLoad = function () {
        ...
        loadAndParseURL(image.src)
                .done(function (a) {
                    if (!image.hasAttribute("width") && (!image.style.width || image.style.width == "auto")) image.style.width = w + "px";
                    if (!image.hasAttribute("height") && (!image.style.height || image.style.height == "auto")) image.style.height = h + "px";
    ...
    var animationFound = function() {
        ...
        if (document.images.length == 1 && document.images[0].src == location.href) {
            ...
            cont.querySelector("span").addEventListener("click", function() {
                isHidden = !isHidden;
                if (isHidden) {
                    this.innerHTML = "Show APNG frames";
                    this.className = "";
                    cont2.style.display = "none";
                } else {
                    this.innerHTML = "Hide APNG frames";
